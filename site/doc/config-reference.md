...
| `keepalive_interval` | `string` | no | The WireGuard persistent-keepalive interval (time.ParseDuration) applied to enrolled peers in both directions and pushed to the sidecar at enroll. Optional; defaults to 25s and must be between 10s and 25s. The 25s ceiling is a WireGuard rekey-safety bound: on an idle tunnel a keepalive must arrive before the session expires or the peer loses liveness and self-heals, so lengthen the liveness window with keepalive_reap_count rather than a larger interval. |
