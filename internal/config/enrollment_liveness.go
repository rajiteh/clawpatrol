...
const (
	...
	// There is no upper bound on keepalive_interval. An earlier build capped it
	// at 25s for WireGuard rekey safety — the sidecar is the handshake
	// initiator, and a too-coarse keepalive could let an idle session lapse
	// before a rekey. That is no longer a liveness concern: the sidecar is now
	// the sole (authoritative) keepalive sender (so the reaper never loses a
	// timer race and false-reaps) and checks its own direction with an active
	// ICMP probe that also drives rekeying off real traffic, self-healing any
	// lapse in process. A longer interval simply means fewer keepalive packets
	// and a longer liveness window (keepalive_interval × keepalive_reap_count) —
	// a policy choice left to the operator.
	//
