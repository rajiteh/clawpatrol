...
- The Kubernetes sidecar is `clawpatrol run --tun`, not a dedicated
  subcommand. In Kubernetes manifests it is declared as a native sidecar
  init container (`initContainers` + `restartPolicy: Always`). `--tun`
  realizes the data plane as a real TUN with netns-wide routing
  (privileged); `--dynamic-peer-authorizer <type>/<name>` self-registers
  as a dynamic peer and renews the lease. The two flags are independent
  identity/transport axes; the privileged sidecar uses them together. The
  register request sends:
  - `transport = "wireguard"`
...
Verification reality has changed across workspaces. Go unit tests and
Kubernetes dry-runs have been exercised locally in the current iteration;
dashboard formatting/build checks still require the dashboard toolchain, and
full kind e2e validation requires Docker plus access to the local kind API.
