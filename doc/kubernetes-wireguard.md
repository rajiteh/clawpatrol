...
WireGuard server. Agent pods use a native Kubernetes sidecar
init container running `clawpatrol run --tun` to create the pod-level
WireGuard tunnel; the execution container remains unprivileged.
...
6. The sidecar fetches `/api/env-pushdown`, writes `/clawpatrol/env` and
   `/clawpatrol/ca.crt`, writes `/clawpatrol/ready`, passes its startup
   probe, and heartbeats until shutdown.
